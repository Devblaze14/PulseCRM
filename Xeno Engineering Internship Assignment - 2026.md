...
Xeno helps consumer brands reach their shoppers in meaningful, data-driven ways — organising customer data, deciding who to talk to, and running personalised campaigns across channels like **WhatsApp, SMS, Email and RCS** . This assignment gives you a taste of the kind of work we do every day, and gives us a window into how you think, build, and ship. 
...
**Build and deploy an AI-native Mini CRM that helps a brand intelligently reach its shoppers.** 
...
- **Segment shoppers** — let the marketer (or your AI) carve out audiences from that data based on behaviour and attributes. 

- **Send personalised communications** — dispatch tailored messages to a chosen audience through a channel service (see below). 
...
The word “intelligent” above is intentional. We want to see AI woven into the product itself, not bolted on. There is no single right shape for this — and we expect different candidates to make very different bets. For example: 

- **A classic product interface** where AI assists the marketer at key steps (e.g. drafting messages, suggesting segments). 

- **A chat-first experience** where the marketer describes intent in natural language and the product responds. 

- **An AI that helps the marketer think, decide and act** — surfacing the right audience, recommending the message, choosing the channel. 

- **A true AI agent** that takes a broad goal brainstormed with the marketer and then executes the campaign end to end. 
...
- Your CRM exposes a **send API** . When a campaign goes out, the CRM calls a **separate stubbed channel service** with the communication details (recipient, message, channel). 

- The channel service **does not actually deliver anything** — it simulates outcomes. Asynchronously, it calls back into a **CRM receipt API** with what “happened” to each communication: **delivered, failed, opened, read, clicked** , and so on. 

- The CRM ingests these callbacks and updates the state and stats of each communication accordingly. 
...
|---|---|
|Build & deploy (table<br>stakes)|A live, hosted, working product with a walkthrough video. This<br>is the baseline expectation, not a differentiator — you stand<br>out on everything below.|
|Creativity in scoping|How sharply you figured out WHAT to build. The brief is<br>intentionally open. The best submissions make bold,<br>opinionated product choices rather than building everything<br>shallowly.|



|**What we evaluate**|**What we're looking for**|
|---|---|
|AI-native development|How well you leveraged AI to build. We want to see an<br>AI-native workflow — how you direct, review, and integrate AI<br>output, not whether you used it.|
|Code quality & structure|Clean, readable, well-organised code with sensible structure.<br>We will read your code and ask about it.|
|System design & scalability|Your scale assumptions and the tradeoffs you consciously<br>made. We care about your reasoning more than a perfect<br>architecture.|
|Thought clarity &<br>communication|How clearly you think, present, and explain — in your video<br>and in your decisions.|



**In short:** a working, deployed product is the baseline. You stand out on the creativity of what you chose to build, how AI-native your approach and your workflow were, the quality of your code and system thinking, and how clearly you can present it all. 
...
|**Section**|**Suggested**<br>**time**|**What to cover**|
|---|---|---|
|Product intro|~0.5 min|What you built and why. The problem you chose to<br>solve.|
|Functional demo|~1.5 min|End-to-end product demo. Show it actually working.|
|Technical architecture|~1 min|Your architecture diagram and the reasoning behind<br>each major decision.|
|Code walkthrough|~1 min|Walk us through the structure and a couple of key<br>parts of your code.|
|AI-native workflow|~1 min|How you leveraged AI while building — how AI-native<br>your dev workflow was.|



## **A Few Ground Rules** 

- **Use AI freely.** We actively want you to. We are an AI-native company and we expect our engineers to be AI-native too. Just be ready to explain and defend everything you ship — we will ask. 
...
Submissions are due by **12 PM on June 15, 2026** . Submit your hosted URL, repository link, and walkthrough video via the following submission form. 

Submission link for FDE candidates Submission link for SDE candidates 
